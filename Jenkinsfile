        ...
        withCredentials([
          ...
          withCredentials([sshUserPrivateKey(credentialsId: "${SSH_CRED_ID}", keyFileVariable: 'Taxonomy-vm-ssh-key', usernameVariable: 'SSH_USER')]) {
            sh '''
              ...
              scp -i $SSH_KEY StrictHostKeyChecking=no $SSH_USER@$TARGET_IP "/var/lib/jenkins/workspace/Taxonomy_NEW/scripts/collect_agent.py $SSH_USER@$TARGET_IP:/home/$SSH_USER/forensic/collect_agent.py"
