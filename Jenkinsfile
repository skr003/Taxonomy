    ...
    stage('Upload Reports to Azure Storage') {
      steps {
            ...
            withCredentials([
                ...
                az storage blob upload --container-name $CONTAINER --name "builds/$BUILD_NUMBER/artifacts.json" --file forensic_workspace/*.json --account-name $STORAGE_ACCOUNT --account-key "$TAXONOMY_STORAGE_ACCOUNT_KEY" --overwrite
