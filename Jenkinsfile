    ...
    stage('Upload Reports to Azure Storage') {
      steps {
            ...
            withCredentials([
                ...
                az storage blob upload-batch --account-name $STORAGE_ACCOUNT --account-key "$TAXONOMY_STORAGE_ACCOUNT_KEY" --container-name $CONTAINER --destination "builds/$BUILD_NUMBER" --source forensic_workspace
                az storage blob upload-batch --account-name $STORAGE_ACCOUNT --account-key "$TAXONOMY_STORAGE_ACCOUNT_KEY" --container-name $CONTAINER --destination "latest" --source forensic_workspace
